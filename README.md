...
│
├── docs/
│   └── GASOLINE_DETAIL_DOCUMENTATION.md # เอกสารระบบ Gasoline Detail สถาปัตยกรรม Tech Stack และ Sources
│
...
| **`Yesterday’s Completed Jobs`** | `sync-yesterday-jobs.js` | 30 Parent / 30 Sub | เมื่อวาน (วันเดียว) หรือระบุวันแบบ Manual | ใช้ **`date_type=2`** (ค้นหาจากวันที่นัดหมาย)<br>จัดเรียงตามเวลานัดหมาย (เช้า ➔ เย็น)<br>ดึงข้อมูลงานที่มีนัดหมายเมื่อวาน (รองรับระบุวันที่ Manual ผ่าน `target_date`) |
...
| **`Gasoline Detail`** | `sync-gasoline.js` | 4 ทีม (Batch) | 1 เดือนล่าสุด + เดือนปัจจุบัน (วันที่ 1 เดือนที่แล้ว - วันนี้) | ดึง `.xlsx` สรุปรายบุคคลของ 4 ทีม<br>ล้างชีทก่อนเขียนใหม่ทุกรอบ พร้อมคงค่า Review/Note เดิม<br>**All Technicians (Col F):** ดึงช่างทั้งหมดในงานจาก Trick2 หรือ Rocket<br>**Auto-Backfill:** หลัง sync ตรวจหาช่อง All Techs และ URL ที่ว่าง แล้วเติมให้อัตโนมัติ |
